    ...
    pkgs.writeText "CommonTasks.yml" (
      builtins.toJSON {
        ...
        vars = with hostConfig.config.lollypops; {
          ...
          REMOTE_SSH_OPTS = ''{{default "${lib.concatStrings deployment.ssh.opts}" .LP_REMOTE_SSH_OPTS}}'';
          REMOTE_SUDO_COMMAND = ''{{default "${deployment.sudo.command}" .LP_REMOTE_SUDO_COMMAND}}'';
          REMOTE_SUDO_OPTS = ''{{default "${lib.concatStrings deployment.sudo.opts}" .LP_REMOTE_SUDO_OPTS}}'';
